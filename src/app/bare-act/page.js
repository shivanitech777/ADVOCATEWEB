            ...
            <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
                ...
                >
                    <div className="relative max-w-2xl mx-auto">
                        ...
                        <input
                            type="text"
                            placeholder="Search Bare Act..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-12 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:border-[#C5A25A] focus:outline-none text-gray-700 text-lg transition-all duration-300"
                        />
                    </div>
                    <p className="text-center text-gray-500 mt-4">
                        {filteredActs.length} {filteredActs.length === 1 ? 'act' : 'acts'} found
                    </p>
                </motion.div>
                        ...
                        {filteredActs.length > 0 ? (
                            filteredActs.map((act, index) => (
                                <motion.div
                                    ...
                                    onClick={() => handleActClick(act.pdf)}
                                    className="group cursor-pointer hover:bg-gradient-to-r hover:from-[#C5A25A]/5 hover:to-transparent transition-all duration-300 px-6 py-4 flex items-center justify-between"
                                >
                                    <div className="flex items-center gap-4 flex-1">
                                        <div className="bg-[#C5A25A]/10 group-hover:bg-[#C5A25A]/20 p-3 rounded-lg transition-all duration-300">
                                            <FileText className="w-5 h-5 text-[#C5A25A]" />
                                        </div>
                                        <span className="text-gray-800 font-medium text-base md:text-lg group-hover:text-[#C5A25A] transition-colors duration-300">
                                            {act.name}
                                        </span>
                                    </div>
                                    <ExternalLink className="w-5 h-5 text-gray-400 group-hover:text-[#C5A25A] opacity-0 group-hover:opacity-100 transition-all duration-300" />
                                </motion.div>
                            ))
                        ) : (
                ...
                <motion.div
                    ...
                    className="mt-12 bg-gradient-to-r from-[#C5A25A]/10 to-[#B8954D]/10 rounded-2xl p-8 text-center"
                >
